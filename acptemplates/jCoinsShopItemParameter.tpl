...
<dl>
        ...
        <dd>
            ...
            {elseif $parameter.type == 'BOOL'}
                <label><input id="parameter{$parameter.parameterID}" type="checkbox" name="param_{$parameter.parameterID}_{$parameter.name}"{if $values[$parameter.parameterID]|isset && $values[$parameter.parameterID] == 1} checked="checked"{/if} value="1"> {$parameter.name}</label>
            {/if}
